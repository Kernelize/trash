...
class Server {
public:
  Server(IoQueue &io, Fd &&socket) : io_(io), socket_(std::move(socket)) {}
...
private:
  void receive() {
    ...
    io_.recvfrom(socket_, receiveBuffer_.data(), receiveBuffer_.size(), 0,
        ...
        .callback([this](IoResult receivedBytes) {
          if (!receivedBytes) {
            spdlog::error("Error in recvfrom: {}",
                          receivedBytes.error().message());
            receive();
            return;
          }
  ...
  void respond() {
    io_.sendto(socket_, receiveBuffer_.data(), receiveBuffer_.size(), 0,
               &clientAddr_)
