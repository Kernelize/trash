...
static_assert(std::is_same_v<std::tuple<std::tuple<>, int32_t>,
...
constexpr std::invoke_result_t<F, Args...> invoke_(
...
using InternalPatternT =
    std::remove_reference_t<AddConstToPointerT<decayArrayT<Pattern>>>;
...
class PatternPair {
 ...
 private:
  ...
  std::conditional_t<std::is_function_v<Func>, Func const &, Func const>
      mHandler;
};
...
struct StorePointer<Type, Value,
                    std::void_t<decltype(std::declval<ValueVariant<Type> &>() =
                                             &std::declval<Value>())>>
    : std::is_reference<Value>  // need to double check this condition. to
                                // loosen it.
{};

static_assert(!StorePointer<char, char>::value);
// static_assert(StorePointer<std::tuple<char, int>, std::tuple<char,
// int>>::value);
static_assert(StorePointer<char, char &>::value);
...
class IdBlock : public IdBlockBase<Type> {
 public:
  ...
  constexpr decltype(auto) get() const {
    return std::visit(overload([](Type const &v) -> Type const & { return v; },
...
class IdBlock<Type &> : public IdBlockBase<Type &> {
 public:
  constexpr auto hasValue() const {
    return std::visit(overload([](Type *) { return true; },
                               [](std::monostate const &) { return false; }),
                      IdBlockBase<Type &>::mVariant);
  }
...
class IdBlock<Type &&> : public IdBlockBase<Type &&> {
 public:
  constexpr auto hasValue() const {
    return std::visit(
        overload([](Type const &) { return true; }, [](Type *) { return true; },
                 [](std::monostate const &) { return false; }),
        IdBlockBase<Type &&>::mVariant);
  }
...
template <std::size_t valueStartIdx, std::size_t patternStartIdx,
          ...
          typename ContextT>
constexpr decltype(auto) matchPatternMultipleImpl(ValueTuple &&valueTuple,
...
template <std::size_t valueStartIdx, std::size_t patternStartIdx,
          ...
          typename ContextT>
constexpr decltype(auto) matchPatternMultiple(ValueTuple &&valueTuple,
                                              PatternTuple &&patternTuple,
                                              int32_t depth,
                                              ContextT &context) {
...
class PatternTraits<PostCheck<Pattern, Pred>> {
 public:
  ...
  constexpr static auto matchPatternImpl(
      Value &&value, PostCheck<Pattern, Pred> const &postCheck, int32_t depth,
      ContextT &context) {
...
constexpr auto matchPatterns(Value &&value, PatternPairs const &...patterns) {
  ...
  if constexpr (!std::is_same_v<RetType, void>) {
    constexpr auto const func = [](auto const &pattern, auto &&value,
                                   RetType &result) constexpr -> bool {
...
class AsPointer {
 ...
 public:
  template <typename Variant,
            typename std::enable_if<viaGetIfV<T, std::decay_t<Variant>>>::type
                * = nullptr>
  ...
  template <typename A,
            typename std::enable_if<std::is_same<
                std::decay_t<A>, std::any>::value>::type * = nullptr>
  ...
  // cast to base class
  template <typename D,
            typename std::enable_if<!viaGetIfV<T, D> &&
                                    std::is_base_of_v<T, D>>::type * = nullptr>
  ...
  // No way to handle rvalue to save copy in this class. Need to define some in
  // another way to handle this. cast to base class
  template <typename D,
            typename std::enable_if<!viaGetIfV<T, D> &&
                                    std::is_base_of_v<T, D>>::type * = nullptr>
  ...
  template <typename B,
            typename std::enable_if<!viaGetIfV<T, B> &&
                                    std::is_base_of_v<B, T>>::type * = nullptr>
  ...
  constexpr auto operator()(T const &b) const { return std::addressof(b); }
...
template <typename T>
constexpr auto as = [](auto const pat) { return app(asPointer<T>, some(pat)); };
